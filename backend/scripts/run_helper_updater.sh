...
if [[ "$ENV_CHOICE" == "prod" ]]; then
    export JUPYTER_URL="https://3.111.135.132:9944/notebooks/LoadHelperCode_to_FunctionBased.ipynb"
    export JUPYTER_PASSWORD=${PROD_J}
else
    export JUPYTER_URL="https://3.111.135.132:2222/notebooks/debugers.ipynb"
...
if [ -z "$JUPYTER_PASSWORD" ]; then
    ...
    pipeline_exception "PHASE_2_PERFORM_ACTIONS" "run_helper_updater.sh (credentials)" 1 "JUPYTER_PASSWORD empty — set BETA_JUPYTER_PASSWORD in .secrets.env or use SKIP_JUPYTER=1"
fi
